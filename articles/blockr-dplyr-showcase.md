...
Use the column selector to pick the columns you want. You can select
multiple columns and reorder them by dragging. The order of selection
determines the column order in the output.

The block includes a “distinct” option. When enabled, duplicate rows are
removed from the result, keeping only unique combinations of the
selected columns.

![Select block interface](../reference/figures/select-block.png)
...
Combine multiple conditions using `&` (AND) to require all conditions to
be true, or `|` (OR) to require at least one condition to be true. The
expression editor provides syntax highlighting and validates your
...
When sorting by multiple columns, the order matters. The first column is
the primary sort key. Rows with the same value in the first column are
...
![Arrange block interface](../reference/figures/arrange-block.png)
...
(random selection), or custom (specific positions).

For head and tail types, specify the number of rows using n (count) or
prop (proportion between 0 and 1). For min and max types, select an
...
for weighted sampling and enable replace for sampling with replacement.

The custom type accepts a rows expression like “1:5” or “c(1, 3, 5,
10)”. All slice types support grouping via the by parameter, which
performs the slice operation within each group separately.

![Slice block interface](../reference/figures/slice-block.png)
...
[`round()`](https://rdrr.io/r/base/Round.html), etc.) in your
expressions. Reference existing columns by name. You can also use
conditional logic with [`ifelse()`](https://rdrr.io/r/base/ifelse.html)
or
...
← old_name” with a visual arrow indicator.
...
![Rename block interface](../reference/figures/rename-block.png)
...
Join types: left_join keeps all rows from the left dataset and matching
rows from the right; right_join keeps all rows from the right dataset
and matching rows from the left; inner_join keeps only rows that match
in both datasets; full_join keeps all rows from both datasets; semi_join
filters the left dataset to rows that have a match in the right;
...
Columns are matched by name. If datasets have different columns, the
...
All input datasets must have exactly the same number of rows. The rows
are combined by position: the first row from each dataset forms the
...
Select which columns to pivot. These columns are transformed into two
...
row-column combinations become cells.

Select which column contains values for new column names (names_from)
