...
test_that("parse_value_filter function", {
  ...
  expect_true(grepl("Species.*%in%.*\"setosa\".*&.*!.*Sepal.Length.*%in%.*5.1", expr_str))
})
  ...
  testServer(
    ...
    ),
    {
      ...
      expect_equal(
        ...
        sort(c("Sepal.Length", "Sepal.Width", "Petal.Length", "Petal.Width", "Species"))
      )
      
      # Test unique value extraction for Species (factor column)
      species_values <- get_unique_values("Species")
      expect_equal(sort(species_values), sort(c("setosa", "versicolor", "virginica")))
      
      # Test unique value extraction for numeric column
      sepal_length_values <- get_unique_values("Sepal.Length")
      expect_true(is.numeric(sepal_length_values) || all(grepl("^[0-9.]+$", sepal_length_values)))
...
test_that("mod_value_filter_server handles initial conditions", {
  ...
  testServer(
    ...
    ),
    {
      ...
      conditions <- session$getReturned()()
