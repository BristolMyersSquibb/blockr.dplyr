...
The filter block keeps only rows that meet specific conditions. Enter logical expressions using column names and comparison operators. This block is designed for users who want to write filter conditions as expressions.
...
For more complex filter conditions using comparisons or calculations, use the filter block instead. The filter block allows you to write expressions like `mpg > 20` or `hp / wt > 50` which cannot be expressed through value selection.

```{r value-filter-block, out.width="100%", fig.cap="Value filter block interface"}
knitr::include_graphics("../man/figures/value-filter-block.png")
```
...
knitr::include_graphics("../man/figures/mutate-block.png")
```
