...
register_dplyr_blocks <- function() {
  register_blocks(
    ...
    description = c(
      ...
      "Create new columns or modify existing ones using R expressions. (dplyr: mutate)",
      "Calculate totals, averages, counts, and other statistics. Simple mode or R expressions. (dplyr: summarize)",
    ...
    arguments = list(
      ...
      structure(
        c(
          state = paste0(
            "Object with: columns (array of column name strings), ",
            "exclude (boolean), distinct (boolean)"
          )
        ),
        examples = list(
          state = list(columns = list("mpg", "cyl", "hp"), exclude = FALSE, distinct = FALSE)
        )
      ),
      # join_block:
      structure(
        c(
          state = paste0(
            "Object with: type (join type string), keys (array of {xCol, op, yCol}), ",
            "exprs (array of R expression strings), suffix_x, suffix_y"
          )
        ),
        examples = list(
          state = list(type = "left_join", keys = list(list(xCol = "id", op = "==", yCol = "id")),
      ...
      structure(
        c(
          state = 'Object with: columns (array of {column, direction} where direction is "asc" or "desc")'
        ),
        examples = list(
          state = list(columns = list(list(column = "mpg", direction = "desc")))
        )
      ),
      # mutate_block:
      structure(
        ...
        prompt = paste(
          "Expressions are R code strings. Each row creates or modifies a column.",
      ...
      structure(
        c(
          state = paste0(
            "Object with: summaries (array of summary objects) and by (array of grouping column names). ",
            "Summary types: 'simple' (name, func like 'mean'/'sd'/'sum'/'n'/'n_distinct', col) ",
            "or 'expr' (name, expr as R expression string)"
          )
        ),
        examples = list(
          state = list(
        ...
        prompt = paste(
          "Simple mode uses predefined functions (mean, median, sd, min, max, sum, n, n_distinct, first, last).",
          "Expression mode allows arbitrary R code.",
          "The 'n' function does not need a column argument."
        )
      ),
      # filter_block:
      structure(
      ...
      # bind_rows_block:
      structure(
        c(state = "Object with: id_name (optional string for .id column name)"),
        examples = list(state = list(id_name = "source"))
      ),
      # bind_cols_block:
      NULL,
      # rename_block:
      structure(
        c(state = "Object with: renames (object where keys are new names, values are old names)"),
        examples = list(state = list(renames = list(miles_per_gallon = "mpg", cylinders = "cyl")))
      ),
      # slice_block:
      structure(
        c(
          state = paste0(
            "Object with: type ('head'/'tail'/'min'/'max'/'sample'), n (integer), ",
            "prop (number 0-1, alternative to n), order_by (column for min/max), ",
            "with_ties (boolean), weight_by (column for sample), replace (boolean), ",
            "by (array of grouping columns)"
          )
        ),
        examples = list(
          state = list(type = "head", n = 10L, prop = NULL, order_by = "",
      ...
      structure(
        c(
          state = paste0(
            "Object with: cols (array of column names to pivot), names_to (string), ",
            "values_to (string), values_drop_na (boolean), names_prefix (string)"
          )
        ),
        examples = list(
          state = list(cols = list("col_a", "col_b"), names_to = "variable",
      ...
      structure(
        c(
          state = paste0(
            "Object with: names_from (array), values_from (array), id_cols (array, optional), ",
            "values_fill (value or null), names_sep (string), names_prefix (string)"
          )
        ),
        examples = list(
          state = list(names_from = list("category"), values_from = list("value"),
      ...
      structure(
        c(
          state = paste0(
            "Object with: col (new column name), cols (array of columns to unite), ",
            "sep (separator string), remove (boolean), na_rm (boolean)"
          )
        ),
        examples = list(
          state = list(col = "full_name", cols = list("first_name", "last_name"),
                       sep = " ", remove = TRUE, na_rm = FALSE)
        )
      ),
      # separate_block:
      structure(
        c(
          state = paste0(
            "Object with: col (source column), into (array of new column names), ",
            "sep (separator string), remove (boolean), convert (boolean)"
          )
        ),
        examples = list(
          state = list(col = "full_name", into = list("first", "last"), sep = " ",
