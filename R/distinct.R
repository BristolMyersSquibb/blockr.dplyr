...
#' Run standalone distinct example
#'
#' @export
run_distinct_example <- function() {
  ui <- bslib::page_fluid(
    theme = bslib::bs_theme(version = 5),
    h2("Distinct Block Example"),
    div(
      class = "row",
      div(
        class = "col-md-6",
      ...
      div(
        class = "col-md-6",
        h3("Distinct Data"),
        tableOutput("distinct_data")
      )
    ),
    hr(),
    uiOutput("distinct_ui")
  )
  ...
  server <- function(input, output, session) {
    ...
    sample_data <- reactive({
      data.frame(
        category = c("A", "B", "A", "B", "A", "C", "A", "B"),
        value = c(10, 20, 10, 20, 10, 30, 15, 20),
        id = c(1, 2, 3, 4, 5, 6, 7, 8),
        stringsAsFactors = FALSE
      )
    })

    # Create distinct block
    distinct_block <- new_distinct_block()

    # Call the block's server function
    block_result <- distinct_block$server(
      id = "distinct",
      data = sample_data
    )
    ...
    output$distinct_ui <- renderUI({
      distinct_block$layout("distinct")
    })
    ...
    output$distinct_data <- renderTable({
      req(block_result$result())
      block_result$result()
    })
  }
